...
# Modified entrypoint script that uses npm instead of pnpm
# This resolves the pnpm EACCES permission issues
...
start_server() {
    ...
    if [ -n "$START_COMMAND" ]; then
        ...
        echo "Using npm dev (instead of pnpm due to permission issues)"
        npm run dev &
    fi
    SERVER_PID=$!
